...
const ARCHIVE_HANDLER = 'archive'
const ARCHIVE_ARGS_INDEX = 'index'

function archiveContentArgs(path: string) {
  return `content:${path}`
}

function archivePackArgs(members: string[]) {
...
const load = async () => {
  ...
  try {
    ...
    const result = await getArtifact<ArchiveEntry[]>(
      props.entry.path,
      props.entry.meta,
      ARCHIVE_HANDLER,
      { ref: info.ref }
