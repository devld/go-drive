...
- **权限控制** —— 基于用户与组的细粒度路径权限控制。
- **路径挂载** —— 将任意存储或子路径挂载到统一的虚拟目录树中。
- **媒体预览** —— 基于 [PhotoSwipe](https://github.com/dimsemenov/PhotoSwipe) 的图片画廊、内置音乐播放器以及视频播放。
...
- **定时任务** —— 基于 [gocron](https://github.com/go-co-op/gocron) 的 cron 风格后台任务。
