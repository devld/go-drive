...
func TestZipProducePacksDriveFiles(t *testing.T) {
	...
	args, e := EncodeArgs(Selection{Paths: []string{"a.txt"}})
	if e != nil {
		t.Fatal(e)
	}
...
func TestZipKeepsMemberPathRelative(t *testing.T) {
	...
	args, e := EncodeArgs(Selection{Paths: []string{"docs/a.txt"}})
	if e != nil {
		t.Fatal(e)
	}
...
func TestZipRejectsOversizedSelection(t *testing.T) {
	drive := newZipDrive()
	if _, e := drive.Save(task.DummyContext(), "a.txt", 4, true, bytes.NewReader([]byte("abcd"))); e != nil {
		t.Fatal(e)
	}
	source, _ := drive.Get(context.Background(), "")
	handler, _ := NewHandler(artifact.HandlerContext{Config: common.Config{}})
	args, e := EncodeArgs(Selection{Paths: []string{"a.txt"}, MaxBytes: 1, MaxLabel: "1"})
	if e != nil {
		t.Fatal(e)
	}
	errProduce := handler.(*Handler).Produce(task.DummyContext(), artifact.Request{Source: source, Args: args}, bufferWriter{Buffer: &bytes.Buffer{}})
