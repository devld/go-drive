// Modified copy of github.com/t3rm1n4l/go-mega. See LICENSE and README.
// Copyright (c) 2019 Sarath Lakshman
// Modifications copyright (c) 2026 devld
...
type Mega struct {
	...
	// Cancels the server-event poll started by getFileSystem.
...
// waitOrDone sleeps for d, or returns false when ctx is cancelled.
...
// Close stops the background server-event poller started after login.
func (m *Mega) Close() {
	if m.pollCancel != nil {
		m.pollCancel()
	}
...
// pollEvents listens for server event notifications and applies them.
func (m *Mega) pollEvents(ctx context.Context) {
	...
	for {
		...
		if events.W != "" {
			...
			waitReq, waitErr := http.NewRequestWithContext(ctx, http.MethodGet, events.W, nil)
			if waitErr != nil {
				err = waitErr
				continue
			}
